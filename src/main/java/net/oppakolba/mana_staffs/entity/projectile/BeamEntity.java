package net.oppakolba.mana_staffs.entity.projectile;
...
import net.oppakolba.mana_staffs.item.staffs.BeamStaff;
...
public class BeamEntity extends Entity {
    ...
    private void cleanUpBeamId() {
        if (this.owner instanceof Player player) {
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                ...
                if (stack.getItem() instanceof BeamStaff) {
    ...
    private boolean shouldBeRemoved() {
        ...
        if (!(usingItem.getItem() instanceof BeamStaff)) {
            this.cleanUpBeamId();
            return true;
        }
