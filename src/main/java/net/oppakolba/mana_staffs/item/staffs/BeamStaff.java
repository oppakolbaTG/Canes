...
@Slf4j
public class BeamStaff extends StaffsItem {
    ...
    public BeamStaff(Properties pProperties) {
            ...
            if (!level.isClientSide) {
                if (count % 2 == 0) {
                    ...
                    int currentMana = StaffsItem.getMana(stack);
                        if (StaffsItem.getMana(stack) >= 2) {
                            Vec3 lookVec = player.getLookAngle();
                            Vec3 playerEyePos = player.getEyePosition();
                            Vec3 pLookPos = playerEyePos.add(lookVec.scale(5.0));
                            if (count % 6 == 0) {
                                StaffsItem.setMana(stack, currentMana - 1);
                            }
                ...
                if(StaffsItem.getMana(stack) == 2) player.getCooldowns().addCooldown(this, 200);
            } else {
                if (tag.contains("BeamId")) {
