...
import net.oppakolba.oppamod.item.canes.BeamCane;
...
public class BeamEntity extends Entity {
    ...
    private void cleanUpBeamId() {
        if (this.owner instanceof Player player) {
            ...
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                ...
                if (stack.getItem() instanceof net.oppakolba.oppamod.item.canes.BeamCane) {
    ...
    private boolean shouldBeRemoved() {
        ...
        if (!(usingItem.getItem() instanceof net.oppakolba.oppamod.item.canes.BeamCane)) {
            this.cleanUpBeamId();
